...
if (enter != null || whoWeAre.length != '') {
  tiersExpandedContainer = document.querySelector('.fourTiers');
  closeModalBtn = document.querySelectorAll('.tiersExpandedBtn');
  ...
  //Commented out the below on the 26th because it was showing error in wordpress console, may need to comment back out
  tiersExpandedContainer.addEventListener('click', openHomeModal, true);
...
//openingFunctionality for additional photos
if (whatWeDo.length != '') {
  tiersExpandedContainer = document.querySelector('.fourTiers');
  closeModalBtn = document.querySelectorAll('.tiersExpandedBtn');

  function openModal(event) {
    tiersExpandedContainer.removeEventListener('click', openModal, true);
    target = event.target;
    ...
    // closestTierExpanded.forEach(el => el.classList.remove('none'));
    ...
    //listen to close
    closeModalBtn.addEventListener('click', function closeModalExpanded(event) {
      ...
      tiersExpandedContainer.addEventListener('click', openModal, true);
    });
  }
  tiersExpandedContainer.addEventListener('click', openModal, true);
}
